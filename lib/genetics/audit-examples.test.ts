...
describe("major morph pairing examples", () => {
  ...
  it("het Patternless × het Patternless matches Phantom math", () => {
    ...
    expect(p(result, "ヘテロ パターンレス")).toBeCloseTo(0.5);
  });

  it("het Cappuccino × het Cappuccino is incomplete dominant 1:2:1", () => {
    const result = calculatePairing(
      { cappuccino: "het" },
      { cappuccino: "het" },
    );
    expect(p(result, "スーパーカプチーノ")).toBeCloseTo(0.25);
    expect(p(result, "カプチーノ")).toBeCloseTo(0.5);
    expect(p(result, "ノーマル")).toBeCloseTo(0.25);
    expect(
  ...
  it("TUG het × TUG het can produce visual TUG; TUG het × Lava het cannot fuse into one axanthic gene", () => {
    ...
    expect(p(same, "アザンティック")).toBeCloseTo(0.25);
    expect(p(same, "ヘテロ アザンティック")).toBeCloseTo(0.5);

    const cross = calculatePairing(
      { axanthicTug: "het" },
      { axanthicLava: "het" },
    );
    expect(p(cross, "アザンティック")).toBe(0);
  ...
  it("het Charcoal × het Charcoal is classic recessive 1 : 2 : 1", () => {
    ...
    expect(p(result, "ヘテロ チャコール")).toBeCloseTo(0.5);
    expect(p(result, "ノーマル")).toBeCloseTo(0.25);
  });

  it("visual Cappuccino × wild is 100% visual cappuccino, never super", () => {
    const result = calculatePairing({ cappuccino: "visual" }, {});
    expect(p(result, "カプチーノ")).toBeCloseTo(1);
    expect(p(result, "スーパーカプチーノ")).toBe(0);
  });

  it("Lilly White super Cappuccino × wild is フラペチーノ, not ソラク", () => {
    const result = calculatePairing(
    ...
    expect(p(result, "カプチーノ")).toBeCloseTo(0.5);
    expect(p(result, "ソラク")).toBe(0);
  });

  it("Lilly White Super Cappuccino × Super Cappuccino is 50% ソラク : 50% super capp", () => {
    const result = calculatePairing(
      { lillyWhite: "het", cappuccino: "visual" },
      { cappuccino: "visual" },
    );
    expect(p(result, "ソラク")).toBeCloseTo(0.5);
    expect(p(result, "スーパーカプチーノ")).toBeCloseTo(0.5);
  });
  ...
  it("independent loci multiply: LW het × Phantom het vs wild", () => {
    ...
    expect(p(result, "リリーホワイト（ヘテロ ファントム）")).toBeCloseTo(0.25);
    expect(p(result, "リリーホワイト")).toBeCloseTo(0.25);
    expect(p(result, "ヘテロ ファントム")).toBeCloseTo(0.25);
    expect(p(result, "ノーマル")).toBeCloseTo(0.25);
  });
